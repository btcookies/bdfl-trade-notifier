...
def player_label(player: Player | None, player_id: str) -> str:
    """Return the display label for a player, or a placeholder if missing/blank."""
    if player is None or not player.name.strip():
...
def render_asset(code: str, league: LeagueInfo, players: dict[str, Player]) -> str:
    ...
    if parts[0] == "FP" and len(parts) == 4 and parts[2].isdigit() and parts[3].isdigit():
